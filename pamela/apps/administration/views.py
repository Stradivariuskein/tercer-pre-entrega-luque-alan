...
def accountDelete(request, pk):
    account = Acount.objects.get(id=pk)
    user = User.objects.get(id=account.user.id)
    account.delete()
    user.delete()
...
def turnCreate(request): # funcion especial para crear un turno con mas campos para el administrador
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            type_session = Acount.objects.get(user=user_id).type_session
    ...
    return render(request, 'administration/turnUpdate.html', {'form': form})
...
def admin_turn(request, p_week): # muestra por semnas todos los turnos agendados
    ...
    for turn in turns:
        account = Acount.objects.get(user=turn.user_id)
